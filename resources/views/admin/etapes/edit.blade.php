...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('admin.etapes.update', $etape->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select class="form-select @error('preparation_id') is-invalid @enderror" 
                    ...
                    @foreach($preparations as $preparation)
                        <option value="{{ $preparation->id }}" 
                                {{ old('preparation_id', $etape->preparation_id) == $preparation->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="number" 
                       ...
                       id="numero_etape"
                       name="numero_etape"
                       value="{{ old('numero_etape', $etape->numero_etape) }}"
                       min="1"
                       required>
                @error('numero_etape')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror"
                          ...
                          name="description"
                          rows="3"
                          required>{{ old('description', $etape->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
